...
from zettelkasten_mcp.server.mcp_server import create_app_with_health, health_check
...
class TestHealthCheck:
    ...
    @pytest.mark.skip(
        reason="Async test requires pytest-asyncio plugin. Functionality verified via test_health_check_endpoint_accessible."
    )
    async def test_health_check_returns_healthy_status(self):
        ...
        assert (
            response.body
            == b'{"status":"healthy","service":"zettelkasten-mcp","transport":"http"}'
        )
